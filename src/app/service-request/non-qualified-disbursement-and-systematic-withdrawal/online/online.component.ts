...
import { PolicySurrenderType } from 'app/enums/policy-surrender-type';
import { AccountType, DistributionMethodTypeMode, PartialSurrenderAmountSelectionMode, PartialSurrenderAmountType, PaymentFrequencyMode, PaymentStartDateType, ReturnPolicyTypeMode, SystematicWithdrawalAmountSelectionMode, SystematicWithdrawalType } from 'app/enums/service-request-qualified-nonQualified.enum';
...
@Component({
    selector: 'non-qualified-disbursement-systematic-withdrawl-online',
    styles: [],
    templateUrl: 'online.component.html'
})

export class ServiceRequestNonQualifiedDisbursementSystematicWithdrawlOnlineComponent implements OnInit {
    ...
    public model: ServiceRequestNonQualifiedDisbursementSystematicWithdrawalModel = null;
    ...
    public policySurrenderType = PolicySurrenderType;
    ...
    public maxFederalLength = "17";
    public maxStateLength = "17";
    ...
    constructor(
        ...
        this.model = this.model == null ? new ServiceRequestNonQualifiedDisbursementSystematicWithdrawalModel() : this.model;
    ...
    setFormProperties() {
        this.noticeOfTaxDisclosure = this.utility.getConfigText("notice_of_Tax_Non_Qualified_Disbursement");
            ...
            currencySignState: [this.model.currencySignState == "" ? this.defaultCurrencySign : this.model.currencySignState]
                ...
                validator: (formGroup: FormGroup) => {
                    formGroup.controls.partialSurrenderAmount.valueChanges.subscribe(() => {
                        if (formGroup.controls.premiumType.value == PolicySurrenderType.Partial)
                    ...
                    formGroup.controls.paymentStartDate.valueChanges.subscribe(() => {
                        if (formGroup.controls.premiumType.value == PolicySurrenderType.SystematicWithdrawal) {
    ...
    onPremiumTypeChanged(event: any) {
        if (event.value == PolicySurrenderType.Partial) {
            this.initializePartialSurrenderControls();
        }
        else if (event.value == PolicySurrenderType.Full) {
            this.initializeFullSurrenderControls();
        }
        else if (event.value == PolicySurrenderType.SystematicWithdrawal) {
    ...
    public onIncomeWithHoldChanged(event) {
        let controls = this.qualifiedDisbursementSystematicWithdrawlForm.controls;
    ...
    validateSystematicWithdrawlControls() {
        ...
        if (premiumType == PolicySurrenderType.SystematicWithdrawal) {
    ...
    validatePartialSurrenderControls() {
        ...
        let isGrossAmountForPartialSurrender = controls["isGrossAmountForPartialSurrender"];
        let premiumType = controls["premiumType"].value;
    ...
    emitResult(comment: any = "") {
        let result = {
            ...
            NonQualifiedDisbursementSystematicWithdrawlData: this.model,
